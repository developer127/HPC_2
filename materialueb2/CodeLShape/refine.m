function [coordinates,newElements,newMaterial,varargout] ...
           = refineRGB(coordinates,elements,material,varargin)
...
%    = REFINERGB(COORDINATES,ELEMENTS,MATERIAL,DIRICHLET,NEUMANN,MARKED)
% 
...
newMaterial(idx(none)) = material(none);
newMaterial([idx(bisec1),1+idx(bisec1)]) = reshape(material(bisec12,[1,1]),[],1);
newMaterial([idx(bisec12),1+idx(bisec12),2+idx(bisec12)]) ...
    = reshape(material(bisec12,[1,1,1]),[],1);
newMaterial([idx(bisec13),1+idx(bisec13),2+idx(bisec13)]) ...
    = reshape(material(bisec13,[1,1,1]),[],1);
newMaterial([idx(red),1+idx(red),2+idx(red),3+idx(red)]) ...
    = reshape(material(red,[1,1,1,1]),[],1);
